...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        #Omega.show()
        #Xi.show()
        #expand Omega for the next position (add two new zero columns and rows)
        Omega2 = matrix()
        Omega2.zero(dim+2, dim+2)
        for i in range(dim+2):
            if i<2:
                for j in range(dim+2):
                    if j<2:
                        Omega2.value[i][j]=Omega.value[i][j]
                    if j>3:
                        Omega2.value[i][j]=Omega.value[i][j-2]
            if i>3:
                for j in range(dim+2):
                    if j<2:
                        Omega2.value[i][j]=Omega.value[i-2][j]
                    if j>3:
                        Omega2.value[i][j]=Omega.value[i-2][j-2]
        
        
        #expand Xi for the next position
        Xi2 = matrix()
        Xi2.zero(dim+2,1)
        for i in range(dim+2):
            if (i<2):
                Xi2.value[i][0]=Xi.value[i][0]
            if (i>3):
                Xi2.value[i][0]=Xi.value[i-2][0]
                
        
        #Omega2.show()
        #Xi2.show()
        #update Omega2 and Xi2               
        for b in range(4):
            Omega2.value[b][b] +=  1.0 / motion_noise
        for b in range(2):
            Omega2.value[b  ][b+2] += -1.0 / motion_noise
            Omega2.value[b+2][b  ] += -1.0 / motion_noise
            Xi2.value[b  ][0]        += -motion[b] / motion_noise
            Xi2.value[b+2][0]        +=  motion[b] / motion_noise
            
            
        #take out auxilliary matrices Omega3, Xi3, A, B, C
        Omega3 = matrix()
        Xi3 = matrix()
        A = matrix()
        B = matrix()
        ...
        for i in range(dim):
            for j in range(dim):
                Omega3.value[i][j]=Omega2.value[i+2][j+2]                            
        for i in range(dim):
            Xi3.value[i][0]=Xi2.value[i+2][0]
        for i in range(2):
            for j in range(dim):
                A.value[i][j]=Omega2.value[i][j+2]
        for i in range(2):
            for j in range(2):
                B.value[i][j]=Omega2.value[i][j]
        for i in range(2):
            C.value[i][0]=Xi2.value[i][0]
        
        
        #modify Omega and Xi using auxilliary matrices
        Omega = Omega3 - A.transpose()*B.inverse()*A
        Xi = Xi3 - A.transpose()*B.inverse()*C
