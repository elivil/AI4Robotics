...
class robot:
    ...
    def move(self, motion): 
        ...
        beta = (d/self.length) * tan(random.gauss(motion[0],self.steering_noise))
        if abs(beta)>0.001:
            R = d/beta
            cy = self.y + cos(theta)*R
            cx = self.x - sin(theta)*R
            res.y = cy - cos(theta+beta)*R
            res.x = cx + sin(theta+beta)*R
            res.orientation = (theta+beta) % (2*pi)
        else:
            res.x = self.x + d*cos(theta)
            res.y = self.y + d*sin(theta)
            res.orientation = (theta+beta) % (2*pi)
        return res 
    ...
    def sense (self, add_noise = 1):
        ...
        for i in range(len(landmarks)):
            delta_x = -(self.x - landmarks[i][1])
            delta_y = -(self.y - landmarks[i][0])
...
##motions = [[2. * pi / 10, 10.] for row in range(number_of_iterations)]
##
