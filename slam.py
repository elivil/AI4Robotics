...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # Omega = matrix([[0.0 for row in range(dim)] for col in range(dim)])
    # Xi = matrix([[0.0 for row in range(dim)] for col in range(1)])
    Omega_init = matrix([[1,0],[0,1]])
    Omega = Omega_init.expand(dim, dim, [0,1], [0,1])
    Xi_init =  matrix([[world_size / 2.0],[world_size / 2.0]])
    Xi = Xi_init.expand(dim, 1, [0,1], [0])
    #Omega.show()
    #Xi.show()
    for j in range(len(data)):
        ...
        for k in range(len(Z)):
            ...
            lm_x=Z[k][1]
            lm_y=Z[k][2]
            dOmega = matrix([[1.0/measurement_noise, 0, -1.0/measurement_noise, 0],[0,1.0/measurement_noise,0,-1.0/measurement_noise],[-1.0/measurement_noise,0,1.0/measurement_noise,0],[0,-1.0/measurement_noise,0,1.0/measurement_noise]])
            #dOmega.show()
            Omega_sense = dOmega.expand(dim, dim, [i, i+1, 2*N+2*lm_index, 2*N+2*lm_index+1], [i, i+1, 2*N+2*lm_index, 2*N+2*lm_index+1])
            #Omega_sense.show()
            Omega += Omega_sense
            #Omega.show()
            dXi_sense = matrix([[-lm_x/measurement_noise],[-lm_y/measurement_noise],[lm_x/measurement_noise],[lm_y/measurement_noise]])
            dXi = dXi_sense.expand(dim, 1, [i, i+1, 2*N+2*lm_index, 2*N+2*lm_index+1], [0])
            Xi += dXi
            #Xi.show()
        dOmega = matrix([[1.0/motion_noise, 0, -1.0/motion_noise, 0],[0,1.0/motion_noise,0,-1.0/motion_noise],[-1.0/motion_noise,0,1.0/motion_noise,0],[0,-1.0/motion_noise,0,1.0/motion_noise]])
        Omega_move = dOmega.expand(dim, dim, [i, i+1, i+2, i+3], [i, i+1, i+2, i+3])
        Omega += Omega_move
        dXi_move = matrix([[-mov[0]/motion_noise],[-mov[1]/motion_noise],[mov[0]/motion_noise],[mov[1]/motion_noise]])
        dXi_move = dXi_move.expand(dim, 1, [i, i+1, i+2, i+3], [0])
        Xi += dXi_move
